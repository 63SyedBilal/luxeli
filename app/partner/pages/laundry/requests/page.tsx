...
export default function LaundryRequestsPage() {
  ...
  const [requestToChangeStatus, setRequestToChangeStatus] = useState<LaundryRequest | null>(null)
  ...
  const fetchRequests = async () => {
    try {
      ...
      if (priorityFilter && priorityFilter !== "Priority") {
        queryParams.append('priority', priorityFilter.toLowerCase())
      }

      const response = await fetch(`/api/partner/laundary-request?${queryParams}`, {
        headers: {
  ...
  }, [currentPage, itemsPerPage, searchQuery, statusFilter, priorityFilter])

  // Map API request to UI format
  const mapApiRequestToUI = (apiRequest: LaundryRequest, index: number) => {
    ...
    const servicesText = apiRequest.services?.join(", ") || ""

    return {
      id: `#L${String(apiRequest._id || apiRequest.id || index + 1).slice(-3).padStart(3, '0')}`,
      requestId: apiRequest._id || apiRequest.id || '',
      room: apiRequest.roomName,
      guest: apiRequest.residentialName,
      piece: `${apiRequest.piece}x ${servicesText}`,
      pickup: formatPickup(apiRequest.pickup),
      ...
      assignee: apiRequest.assigne?.name || "",
      hasAssignee: !!apiRequest.assigne,
      service: apiRequest.service,
      notes: apiRequest.notes,
      note: apiRequest.notes, // Also map as note for modal compatibility
  ...
  const handleAssignStaff = async (staffId: string) => {
    ...
    try {
      ...
      try {
        ...
        const response = await fetch(`/api/partner/laundary-request/${requestToAssign._original._id}/assign`, {
          method: 'POST',
          headers: {
  ...
  const handleStatusChange = async (newStatus: "new" | "accepted" | "completed" | "no-show" | "canceled") => {
    ...
    try {
      ...
      const response = await fetch(`/api/partner/laundary-request/${requestToChangeStatus._id}/status`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleDeleteRequest = async (requestId: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/partner/laundary-request/${requestId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
        ...
        <div className="px-6">
          ...
          <div className="flex items-center justify-between py-3 border-t border-border">
            <p className="text-sm text-muted-foreground">
              Displaying {uiRequests.length} results out of {totalItems}
            </p>
