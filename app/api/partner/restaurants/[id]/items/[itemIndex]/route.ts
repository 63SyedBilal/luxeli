...
// PATCH /api/partner/restaurants/[id]/items/[itemIndex] - Update item
export const PATCH = withAuth(async (request: AuthenticatedRequest, context?: { params?: { [key: string]: string | string[] } | Promise<{ [key: string]: string | string[] }> }) => {
  try {
    ...
    if (context?.params) {
      ...
      itemIndex = parseInt(segments[segments.length - 1], 10);
    }
    ...
    return await RestaurantController.updateItem(id, itemIndex, body);
  } catch (error: any) {
...
// DELETE /api/partner/restaurants/[id]/items/[itemIndex] - Delete item
export const DELETE = withAuth(async (request: AuthenticatedRequest, context?: { params?: { [key: string]: string | string[] } | Promise<{ [key: string]: string | string[] }> }) => {
  try {
    ...
    return await RestaurantController.deleteItem(id, itemIndex);
  } catch (error: any) {
